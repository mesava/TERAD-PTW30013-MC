...
def main() -> None:
    ...
    with Path(args.output).open('w', newline='') as f:
        w = csv.writer(f)
        w.writerow([
            'config','beam','applicator','ssd_cm','field_x_cm','field_y_cm','variant',
            'tio2_mass_fraction','ncase','seed1','seed2','D_RW3_per_history','u_D_RW3_pct',
            'Dcav_RW3_per_history','u_Dcav_RW3_pct','R_RW3','u_R_RW3_abs'
        ])
        w.writerow([
            args.config,args.beam,args.applicator,args.ssd,args.field_x,args.field_y,args.variant,
            args.tio2,args.ncase,args.seed1,args.seed2,d,u_d,c,u_c,r,u_r
        ])
