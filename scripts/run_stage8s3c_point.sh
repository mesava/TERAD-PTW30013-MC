...
: "${BEAM:?}" "${MEDIUM:?}" "${SPECTRUM_VARIANT:?}" "${SEED1:?}" "${SEED2:?}"
...
cd "$GITHUB_WORKSPACE"
mkdir -p stage8s3c_inputs stage8s3c_logs
spec="stage8s3c_spectra/${SPECTRUM_VARIANT}/${BEAM}.ensrc"
test -s "$spec"
cp "$spec" "$EGS_HOME/egs_chamber/${BEAM}.ensrc"

name="stage8s3c_${SPECTRUM_VARIANT}_${BEAM}_${MEDIUM}"
python scripts/build_stage8s3_input.py \
  --template inputs/stage3b_czarnecki_modelA_vrt.template.egsinp \
  --output "stage8s3c_inputs/${name}.egsinp" \
...
input="stage8s3c_inputs/${name}.egsinp"
...
cd "$EGS_HOME/egs_chamber"
set -o pipefail
"$EGS_HOME/bin/gha/egs_chamber" -i "$name" 2>&1 | tee "$GITHUB_WORKSPACE/stage8s3c_logs/${name}.log"
cd "$GITHUB_WORKSPACE"
log="stage8s3c_logs/${name}.log"
...
python scripts/summarize_stage8s3b_point.py \
  --log "$log" \
  --output "stage8s3c_point_${SPECTRUM_VARIANT}_${BEAM}_${MEDIUM}.csv" \
