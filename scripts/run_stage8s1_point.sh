...
cd "$GITHUB_WORKSPACE"
mkdir -p stage8s1_inputs stage8s1_logs
cp "stage8s1_spectra/${BEAM}.ensrc" "$EGS_HOME/egs_chamber/${BEAM}.ensrc"
name="stage8s1_${BEAM}_F50_${VARIANT}"
python scripts/build_stage6_rw3_tio2_sensitivity.py \
  --template inputs/stage3b_czarnecki_modelA_vrt.template.egsinp \
  --output "stage8s1_inputs/${name}.egsinp" \
...
grep -Fq 'bulk density = 1.045000' "stage8s1_inputs/${name}.egsinp"
grep -Fq 'name = chamber_in_rw3' "stage8s1_inputs/${name}.egsinp"
cp "stage8s1_inputs/${name}.egsinp" "$EGS_HOME/egs_chamber/${name}.egsinp"

cd "$EGS_HOME/egs_chamber"
set -o pipefail
"$EGS_HOME/bin/gha/egs_chamber" -i "$name" 2>&1 | tee "$GITHUB_WORKSPACE/stage8s1_logs/${name}.log"
cd "$GITHUB_WORKSPACE"
log="stage8s1_logs/${name}.log"
...
python scripts/summarize_stage8s1_tio2.py \
  --log "$log" \
  --output "stage8s1_point_${BEAM}_${VARIANT}.csv" \
  --beam "$BEAM" --variant "$VARIANT" --tio2 "$TIO2" \
