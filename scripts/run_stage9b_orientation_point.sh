...
: "${BEAM:?}" "${MEDIUM:?}" "${GEOM_ID:?}" "${BASELINE_FIELD:?}" "${ROTATED_FIELD:?}" "${SSD:?}" "${FIELD_X:?}" "${FIELD_Y:?}" "${SEED1:?}" "${SEED2:?}"
...
cd "$GITHUB_WORKSPACE"
mkdir -p stage9b_inputs stage9b_logs
spec="$(find stage9b_spectra -type f -path "*/${SPECTRUM_VARIANT}/${BEAM}.ensrc" | head -n1)"
test -n "$spec" && test -s "$spec"
cp "$spec" "$EGS_HOME/egs_chamber/${BEAM}.ensrc"

name="stage9b_${GEOM_ID}_${BEAM}_${MEDIUM}"
python scripts/build_stage8s3_input.py \
  --template inputs/stage3b_czarnecki_modelA_vrt.template.egsinp \
  --output "stage9b_inputs/${name}.egsinp" \
...
input="stage9b_inputs/${name}.egsinp"
...
# Model B1 longitudinal chamber axis is X by construction.
if 'axis = 2.15 0 0  -1 0 0' not in t:
    raise SystemExit('PTW30013 B1 longitudinal X-axis anchor missing')
PY
...
cd "$EGS_HOME/egs_chamber"
set -o pipefail
"$EGS_HOME/bin/gha/egs_chamber" -i "$name" 2>&1 | tee "$GITHUB_WORKSPACE/stage9b_logs/${name}.log"
cd "$GITHUB_WORKSPACE"
log="stage9b_logs/${name}.log"
...
python scripts/summarize_stage9b_orientation_point.py \
  --log "$log" \
  --output "stage9b_point_${GEOM_ID}_${BEAM}_${MEDIUM}.csv" \
  --beam "$BEAM" --medium "$MEDIUM" --geometry "$GEOM_ID" \
  --baseline-field "$BASELINE_FIELD" --rotated-field "$ROTATED_FIELD" \
