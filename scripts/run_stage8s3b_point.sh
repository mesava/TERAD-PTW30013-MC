...
: "${BEAM:?}" "${MEDIUM:?}" "${SOURCE_MODEL:?}" "${SEED1:?}" "${SEED2:?}"
...
if [[ "$SOURCE_MODEL" != point && "$SOURCE_MODEL" != finite_7p5mm ]]; then
  echo "Invalid SOURCE_MODEL=$SOURCE_MODEL" >&2; exit 2
fi
...
cd "$GITHUB_WORKSPACE"
mkdir -p stage8s3b_inputs stage8s3b_logs
spec="$(find stage8s3b_spectra -type f -path "*/${SPECTRUM_VARIANT}/${BEAM}.ensrc" | head -n1)"
test -n "$spec" && test -s "$spec"
cp "$spec" "$EGS_HOME/egs_chamber/${BEAM}.ensrc"

name="stage8s3b_${BEAM}_${SOURCE_MODEL}_${MEDIUM}"
python scripts/build_stage8s3_input.py \
  --template inputs/stage3b_czarnecki_modelA_vrt.template.egsinp \
  --output "stage8s3b_inputs/${name}.egsinp" \
  --medium "$MEDIUM" --source-model "$SOURCE_MODEL" \
  --beam "$BEAM" --ssd 50 --field-x 8 --field-y 10 \
  --ncase "$NCASE" --nbatch "$NBATCH" --xcse "$XCSE" --rr "$RR" \
  --seed1 "$SEED1" --seed2 "$SEED2"

input="stage8s3b_inputs/${name}.egsinp"
grep -Fq "spectrum file = \$EGS_HOME/egs_chamber/${BEAM}.ensrc" "$input"
if [[ "$SOURCE_MODEL" == finite_7p5mm ]]; then
  grep -Fq 'library = egs_circle' "$input"
  grep -Fq 'radius = 0.375000' "$input"
  grep -Fq 'rectangle = -4.00000000 -5.00000000 4.00000000 5.00000000' "$input"
  grep -Fq 'translation = 0 0 -2.000000' "$input"
else
  grep -Fq 'type = point' "$input"
fi
...
cd "$EGS_HOME/egs_chamber"
set -o pipefail
"$EGS_HOME/bin/gha/egs_chamber" -i "$name" 2>&1 | tee "$GITHUB_WORKSPACE/stage8s3b_logs/${name}.log"
cd "$GITHUB_WORKSPACE"
log="stage8s3b_logs/${name}.log"
...
python scripts/summarize_stage8s3b_point.py \
  --log "$log" \
  --output "stage8s3b_point_${BEAM}_${SOURCE_MODEL}_${MEDIUM}.csv" \
  --beam "$BEAM" --medium "$MEDIUM" --source-model "$SOURCE_MODEL" \
