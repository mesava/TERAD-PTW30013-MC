...
def main() -> None:
    ...
    if len(baseline) != 8:
        raise SystemExit(f'Expected 8 non-F50 Stage 8 baseline rows, found {len(baseline)}')

    points = {}
    for fn in glob.glob(args.points_glob):
        with open(fn, newline='') as f:
            row = next(csv.DictReader(f))
            points[(row['config'], row['variant'])] = row
    if len(points) != 16:
        raise SystemExit(f'Expected 16 sensitivity points, found {len(points)}')
    ...
    for config in sorted(baseline):
        ...
        rows.append([
            config,b['beam'],b['applicator'],b['ssd_cm'],b['field_cm'],k0,
            low[0],low[1],high[0],high[1],endpoint_span_pct,max_shift_pct,
            u_rect_material_pct,span_z_conservative,low[3],high[3]
        ])

    with Path(args.output).open('w', newline='') as f:
        w = csv.writer(f)
        w.writerow([
            'config','beam','applicator','ssd_cm','field_cm','k_nominal',
            'k_tio2_1p6','shift_low_pct','k_tio2_2p4','shift_high_pct',
    ...
    max_u_mat = max(r[12] for r in rows)
